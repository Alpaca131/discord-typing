...
async def game_start(message):
    ...
    game_info.question_index = question_list_index
...
def generate_ranking_embed(game_info: GameInfo):
    ...
    for user_id in game_info.player_list:
        average = numpy.average(game_info.competitor_time[user_id])
    ...
    for val in competitor_ranking:
        ...
        if len(game_info.competitor_time[player_id]) != len(
                game_info.question_list):
            not_answered = str(
                len(game_info.question_list) - len(
                    game_info.competitor_time[player_id])) + '問'
        else:
            not_answered = 'なし'
